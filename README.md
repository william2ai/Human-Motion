...
```bash
code/code/scripts/imputation/daphnet_script/GraphNet_point_ddp_a100.sh
```
...
In DDP mode, `--batch_size` is treated as the global batch size. With `--batch_size 64` and 4 GPUs, each A100 receives a per-process batch size of 16. This keeps the optimization setup comparable to single-GPU experiments.
...
The 4xA100 script writes logs to:
...
- Keep `--learning_rate 0.0001` for the 4xA100 example unless changing the global batch size intentionally.
