...
In DDP mode, `--batch_size` is treated as the global batch size. With `--batch_size 64` and `NUM_GPUS=4`, each process receives a per-GPU batch size of 16. This keeps the optimization setup comparable when you change the GPU count.
